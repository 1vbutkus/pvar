...
This package is a product of my master studies of econometrics in Vilnius University.
...
authors R.~Norvai\v{s}a and A.~Ra\v{c}kauskas presented
necessary and sufficient conditions
for the convergence in law of partial sum processes in
$p$-variation norm. Based on the result of this paper, A.~Ra\v{c}kaskas proposed 
...
The package \emph{pvar} is here to fill that whole.
The algorithm is writen in C++, therefore it is quite fast.
...
The points in supreme partition (the red ones) are actually used in $p$-variation. 
All the other points are redundant and could be doped out with out any effect to $p$-variation.
This could be illustrated, be the example below.
...
To test whether there is actual a multiple break, we can apply \emph{PvarBreakTest} test.
